...
class DeepCBOW(nn.Module):
    ...
    def __init__(
        self, vocab, embedding_dim=300, output_dim=5, hiddens=[100, 100]
    ):
        ...
        self.embed = nn.Embedding(self.vocab_size, embedding_dim, padding_idx=self.vocab.w2i["<pad>"])
...
class DeepCBOWLightning(pl.LightningModule):
    def __init__(self, embedding_dim, vocab, output_dim=5, lr=0.001):
        ...
        self.model = DeepCBOW(vocab, embedding_dim, output_dim)
...
def main():
    ...
    if args.checkpoint:
        ...
        lightning_model = DeepCBOWLightning(args.embedding_dim, loader.vocab, len(i2t), args.lr)

    # prepare a checkpoint to save the best model
    os.makedirs(args.model_dir, exist_ok=True)
    bestmodel_callback = ModelCheckpoint(
        monitor="val_acc",
        save_top_k=1,
        filename="DeepCBOW-{epoch}-{val_loss:.2f}-{val_acc:.2f}",
